...
model = dict(  # Runner arg
    ...
    decode_head=dict(
        ...
        qk_embed_dims=(32, 32, 32, 32),  # r per head, [s4,s3,s2,s1]
        use_coord_bias=True,
        coord_bias_scales=(1.0, 1.0, 1.0, 1.0),
        coord_bias_sigmas=(0.9, 0.7, 0.5, 0.4),
        coord_bias_learnable=True,
        use_edge_bias=False,
        edge_bias_scales=(0.5, 0.5, 0.5, 0.5),
        edge_pool_kernel=3,
        use_agent=True,
        agent_pool_sizes=(8, 8, 8, 8),  # P per stage, tokens=P*P (0 disables)
...
# schedule
schedule_total_step = 200000
schedule_warmup_step = 1500
schedule_val_interval = 1000
schedule_log_interval = 50
schedule_checkpoint_interval = 1000
...
optim_wrapper = dict(  # Runner kwarg
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            'coord_log_sigma': dict(decay_mult=0.),
