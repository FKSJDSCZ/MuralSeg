...
model = dict(  # Runner arg
    ...
    backbone=dict(
        ...
        attn_drop_rate=0.0,
        drop_path_rate=0.1,
        # init_cfg=dict(type='Pretrained', checkpoint=checkpoint),  # mit-b3
        embed_dims=64,  # mit-b3
        num_heads=[1, 2, 5, 8],  # mit-b3
        num_layers=[3, 4, 18, 3],  # mit-b3
        # stages: 0~3: 1/4,1/8,1/16,1/32
        epsr_cfg=dict(
            enable=True,
            stages=(0, 1, 2),
            # blocks={2: list(range(6))}, # stage 2, first 6 blocks
            alpha_init=1.0,
        ...
        msd_cfg=dict(
            enable=True,
            stages=(0, 1, 2, 3),
            # dilations={0:(1,2), 1:(1,2,3), 2:(1,2,3), 3:(1,2)},
            dilations=(1, 3, 5),
        ),
    ),
    decode_head=dict(
        ...
        in_channels=[64, 128, 320, 512],  # mit-b3
        channels=768,  # mit-b3
        num_classes={{_base_.dataset_classes}},
        use_scale_attention=True,  # degenerate to concat+fusion(SegFormerHead) if False
        use_boundary_refine=True,
        boundary_gamma_init=1.0,
        gate_kernel_size=3,
