...
model = dict(  # Runner arg
    ...
    decode_head=dict(
        ...
        # SSMA sources (2D list). Use -1 to include prev-dec (Up(D_{t+1})).
        # This matches the original: source_mode='upto' + include_prev=True
        source_indices=[
            [0, -1],  # stage 0 (H/4):  Z0 + Up(D1)
            [0, 1, -1],  # stage 1 (H/8):  Z0,Z1 + Up(D2)
            [0, 1, 2, -1],  # stage 2 (H/16): Z0,Z1,Z2 + Up(D3)
            [0, 1, 2, 3],  # stage 3 (H/32): Z0,Z1,Z2,Z3
        ],
        downsample_mode='avg',
        use_cross_layer_kv=False,
        cross_layer_kv_depth=-1,
        # SSMA attention
        ssma_num_heads=8,
        ssma_window_size=7,
        ssma_use_window_attn=[True, True, False, False],
        ssma_use_rel_pos_bias=True,
        ssma_sr_ratio=[8, 4, 2, 1],
        # SSMA scale selection
        ssma_mix_mode='softmax',
        ssma_gate_channels=64,
        # SSMA FFN / regularization
        ssma_mlp_ratio=4,
        ssma_attn_drop=0.0,
        ssma_proj_drop=0.0,
        ssma_ffn_drop=0.0,
        ...
        # ELAR
        elar_kernel_size=5,
        elar_guidance_channels=64,
        elar_theta_channels=16,
        elar_use_prob=True,
        elar_prob_channels=16,
        elar_detach_prob=True,
        elar_num_iters=1,
        elar_residual=True,
        elar_residual_weight=1.0,
        # Stage-ELAR
        use_stage_elar=True,
        stage_elar_stages=[0, 1, 2, 3],  # 消融：[] / [1] / [0] / [0,1] / [0,1,2,3]
        stage_elar_low_index=0,  # 用 proj_feats[0] 作为细节 guidance
        stage_elar_kernel_size=[7, 5, 3, 3],  # 按 stage0..3
        stage_elar_guidance_channels=64,
        stage_elar_theta_channels=[8, 16, 16, 16],
        stage_elar_value_channels=[32, 32, 64, 64],  # stage0 value 设小点省算力
        stage_elar_use_prob=False,
        stage_elar_prob_channels=16,
        stage_elar_detach_prob=True,
    ),
