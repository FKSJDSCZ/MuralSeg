...
class ds_conv2d(nn.Module):
    def __init__(
        self, in_planes, out_planes, kernel_size, stride=1,
        ...
        self.weight = nn.Parameter(
            torch.randn(out_planes, in_planes // groups, kernel_size, kernel_size), requires_grad=True,
        )
        if bias:
...
class CSA(nn.Module):
    def __init__(
        self, in_dim, out_dim, num_heads, kernel_size=3, padding=1, stride=2,
    ...
    def forward(self, x, v=None):
        ...
        ).permute(0, 2, 1, 3, 4)  # B,H,N,kxk,kxk
        ...
        v = self.weight(v)
        v = v.reshape(
            B, h * w, self.kernel_size * self.kernel_size, self.num_heads,
               self.out_dim // self.num_heads,
        ).permute(0, 3, 1, 2, 4).contiguous()  # B,H,N,kxk,C/H

        x = (attn @ v).permute(0, 1, 4, 3, 2)
        x = x.reshape(B, self.out_dim * self.kernel_size * self.kernel_size, h * w)
        x = F.fold(
            x, output_size=(H, W), kernel_size=self.kernel_size,
...
class lite_vision_transformer(nn.Module):
    ...
    def __init__(
        self, layers, in_chans=3, num_classes=1000, patch_size=4,
        ...
        if self.with_cls_head:
            ...
            self.downstream_norms = nn.ModuleList(
                [nn.Identity()
                 for idx in range(len(embed_dims))],
            )
        self.apply(self._init_weights)
