...
class Attention4D(torch.nn.Module):
    def __init__(
        self, dim=384, key_dim=32, num_heads=8,
        attn_ratio=4,
        resolution=7,
        act_layer=nn.ReLU,
        stride=None,
        ...
        self.v_local = nn.Sequential(
            nn.Conv2d(
                self.num_heads * self.d, self.num_heads * self.d,
...
class Attention4DDownsample(torch.nn.Module):
    def __init__(
        self, dim=384, key_dim=16, num_heads=8,
        attn_ratio=4,
        resolution=7,
        out_dim=None,
        act_layer=None,
...
class Embedding(nn.Module):
    def __init__(
        self, patch_size=3, stride=2, padding=1,
        ...
        if self.light:
            ...
            self.attn = attn_block(
                dim=in_chans, out_dim=embed_dim,
