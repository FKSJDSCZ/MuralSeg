...
class LKCA2(BaseModule):
    def __init__(
        self, channels1, channels2, norm_cfg=dict(type='SyncBN', requires_grad=True), act_cfg=dict(type='GELU'),
        ...
        for i, (kernel_size, padding) in enumerate(zip(kernel_sizes[1:], paddings[1:])):
            ...
            for i_kernel, i_pad, i_conv in zip(kernel_size_, padding_, conv_name):
                self.add_module(
                    i_conv, nn.Conv2d(channels1, channels1, tuple(i_kernel), padding=i_pad, groups=channels1)
                    )
...
class CCASegHead_mit(BaseDecodeHead):
    def __init__(self, feature_strides, **kwargs):
        super(CCASegHead_mit, self).__init__(input_transform='multiple_select', **kwargs)
        ...
        self.down1_3 = PyramidPoolAgg(stride=8, dim=c1_in_channels)  # feature size = 16*16
        ...
        self.attnc1 = LKCA3(channels1=c1_in_channels, channels2=c4_in_channels)  # feature size = 64*64
        ...
        self.attnc1_1 = LKCA3(channels1=c1_in_channels, channels2=c4_in_channels)  # feature size = 64*64
        ...
        self.attnc1_2 = LKCA3(channels1=c1_in_channels, channels2=c4_in_channels)  # feature size = 64*64

        # Injection Block(Residual = high feature)
        self.inject1 = InjectionMultiSum(dim=c1_in_channels)
        ...
        # Injection Block(Residual = high feature)
        self.multi8_2 = Inject(dim1=c1_in_channels, dim2=c2_in_channels)
        self.multi8_3 = Inject(dim1=c2_in_channels, dim2=c3_in_channels)
        self.multi8_4 = Inject(dim1=c3_in_channels, dim2=c4_in_channels)

        self.multi16_2 = Inject(dim1=c1_in_channels, dim2=c2_in_channels)
        ...
        self.multi32_2 = Inject(dim1=c1_in_channels, dim2=c2_in_channels)
        ...
        self.multi64_2 = Inject(dim1=c1_in_channels, dim2=c2_in_channels)
        ...
        self.predict1_2 = Inject(dim1=c1_in_channels, dim2=c2_in_channels)
    ...
    def forward(self, inputs):
        ...
        # F1-Pooling
        c1_1 = self.down1_1(c1)  # 64x64
        _, _, c1_1_h1, c1_1_w1 = c1_1.shape  # 64x64
        c1_2 = self.down1_2(c1)  # 32x32
        c1_3 = self.down1_3(c1)  # 16x16
        ...
        key_3 = self.multi16_2(c1_2, c2_1)
        ...
        new3_up64 = resize(new3, size=(c1_1_h1, c1_1_w1), mode='bilinear', align_corners=False)
        new4_up64 = resize(new4, size=(c1_1_h1, c1_1_w1), mode='bilinear', align_corners=False)
        key_2 = self.multi32_2(c1_1, c2)
